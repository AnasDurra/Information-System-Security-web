...
, completeInfoSocket, marksSocket } from './services/sockests.js';
  ...
  const navigate = useNavigate();;
    ...
    useEffect(() => {
        if (authToken) {
            ...
            // console.log("connected and auth is :", authToken);
            sethandshakingSocketHeader(authToken);
            handshakingSocket.connect();
            // console.log(handshakingSocket);

            handshakingSocket.on(
                "handshakingResult",
                onCompleteHandshakingResult
            );
            handshakingSocket.on(
                "requestPublicKeyExchange",
                onRequestPublicKeyExchange
            );
            handshakingSocket.on(
                "responseSessionKeyExchange",
                onResponseSessionKeyExchangeResult
            );

            handshakingSocket.on(
                "responseSubmitProjects",
                onResponseSubmitProjects
            );
            ...
            function onRequestPublicKeyExchange(msg) {
                ...
                const ServerPublicKey =
                    forge.pki.publicKeyFromPem(ServerPublicKeyPem);
                ...
                generateSessionKey().then((result) => {
                    ...
                    const encrypted = ServerPublicKey.encrypt(
                        result,
                        "RSA-OAEP"
                    );
        ...
        function onLoginResultEvent(msg) {
            ...
            if (msg.status >= 400 && msg.status < 500)
                console.log("LOGIN REJECTED", msg.error);
